...
def get_config_from_name(cfg, dataset_name):
    ...
    if 'refcoco' in dataset_name:
        ...
        cfg.update(cfg['VOS'])
        return cfg
    elif dataset_name in ['vlp_f30k_train_interleave2i', 'vlp_f30k_val_interleave2i', 'vlp_f30k_test_interleave2i']:
        cfg.update(cfg['F30K'])
        return cfg
...
def build_eval_dataloader(cfg, ):
    ...
    for dataset_name in cfg['DATASETS']['TEST']:
        ...
        elif dataset_name in ['grounding_coco_entity_val', 'grounding_coco_entity_val_long']:
            mapper = GroundingCOCOEntityDatasetMapper(cfg, False)
        elif dataset_name in ["vlp_coco_entity_val", "vlp_coco_entity_val_long", "vlp_coco_entity_val_retrieval", 'vlp_coco_entity_val_retrieval_long']:
            mapper = VLPreCOCOEntityDatasetMapper(cfg, False, dataset_name)
        elif dataset_name in ["vlp_val", "vlp_captioning_val", "vlp_val2017", "vlp_captioning_val2017", "vlp_val_interleave"]:
            mapper = VLPreDatasetMapper(cfg, False, dataset_name)
        elif dataset_name in ["vlp_coco_val_interactive2i"]:
            mapper = VLPreInteractiveDatasetMapper(cfg, False, dataset_name)
        elif dataset_name in ["vlp_coco_interleave_val", "vlp_coco_interleave_val_leak", "vlp_coco_interleave_val_long"]:
            mapper = VLPreCOCOInterleaveDatasetMapper(cfg, False, dataset_name)
        ...
        elif "davis_val" in dataset_name:
            mapper = DAVISDatasetMapperIX(cfg, False, dataset_name)
        elif "ytvos" in dataset_name:
            mapper = YTVOSDatasetMapper(cfg, False, dataset_name)
        elif "openimage600_val" in dataset_name:
            mapper = DAVISDatasetMapperIX(cfg, False, dataset_name)
        elif "cocomini_val" in dataset_name:
            mapper = DAVISDatasetMapperIX(cfg, False, dataset_name)
        elif "ade600_val" in dataset_name:
            mapper = DAVISDatasetMapperIX(cfg, False, dataset_name)
        elif "sbd_val" in dataset_name:
            mapper = SBDDatasetMapper(cfg, False, dataset_name)
        elif 'sun' in dataset_name:
...
def build_train_dataloader(cfg, ):
    ...
    for dataset_name in dataset_names:
        ...
        # coco panoptic segmentation lsj new baseline
            ...
            loaders['coco'] = build_detection_train_loader(cfg, dataset_name=dataset_name, mapper=mapper)
        elif mapper_name == "coco_interleave":
            mapper = COCOPanopticInterleaveDatasetMapper(cfg, True, dataset_name)
            loaders['coco'] = build_detection_train_loader(cfg, dataset_name=dataset_name, mapper=mapper)
        elif mapper_name == "coco_interleave_language":
            mapper = COCOLanguageInterleaveDatasetMapper(cfg, True, dataset_name)
            loaders['coco'] = build_detection_train_loader(cfg, dataset_name=dataset_name, mapper=mapper)
        else:
            mapper = None
...
def build_evaluator(cfg, dataset_name, output_folder=None):
    ...
    # Retrieval
    if evaluator_type in ["retrieval", "retrieval_patch", "retrieval_interactive", "retrieval_interleave_text"]:
        evaluator_list.append(RetrievalEvaluator(dataset_name, output_folder, cfg['MODEL']['DECODER']['RETRIEVAL']['ENSEMBLE']))
    if evaluator_type in ["retrieval_interleave_crossquery"]:
        evaluator_list.append(InterleaveEvaluator(dataset_name, output_folder))
