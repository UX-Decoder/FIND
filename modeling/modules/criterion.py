...
class SetCriterion(nn.Module):
    ...
    def loss_itc_v2(self, outputs, targets, indices, num_masks, layer_id, extra):
        if layer_id >= self.top_x_layers['retrieval']:
            return {"loss_retrieval_decoder_0": 0}

        t_emb = outputs['pred_retrievals_lang'][:,-1]
        v_emb = outputs['pred_retrievals'][:,-1]
        loss_contrast = image_text_contrastive_loss_queue(v_emb, t_emb, extra['lang_encoder'], extra['training'])
        ...
        # losses = {"loss_retrieval_decoder_0": loss_contrast}
        return losses
    ...
    def loss_groundings(self, outputs, targets, indices, num_masks, layer_id, extra):
        ...
        pred_logits = []
        for b in range(len(indices)):
            t_emb = outputs["pred_grounding_query"][b,:len(targets[b]['grounding_hash'])]
    ...
    def loss_spatials(self, outputs, targets, indices, num_masks, layer_id, extra):
        ...
        # return if the layer is before top_x_layers
        ...
        # return if there is no positive mask
        gt_masks = [x['gt_spatial_masks'] for x in targets]
        _stack_gt_mask = torch.cat(gt_masks)
        if _stack_gt_mask.sum() == 0:
            loss = outputs['pred_smasks'].sum() * 0.0
            loss_grd_ce = outputs["pred_smasks"].sum() * 0.0
            return {"loss_spatial_bce_0": loss, "loss_spatial_dice_0": loss, "loss_spatial_ce_0": loss_grd_ce}
        ...
        indices = self.matcher(outputs, targets, mode='spatial')
        src_idx = self._get_src_permutation_idx(indices)
        tgt_idx = self._get_tgt_permutation_idx(indices)

        # recompute a keep index with matched tgt
        stack_gt_mask = nn.utils.rnn.pad_sequence(gt_masks, padding_value=-1).transpose(0,1)[tgt_idx] # ground truth in matching order
        bs,_,_ = stack_gt_mask.shape
        target_masks = stack_gt_mask
        stack_gt_mask = stack_gt_mask.view(bs,-1).sum(dim=-1)
        keep = stack_gt_mask > 0 # only keep sample contain positive mask, keep in matching order

        del stack_gt_mask

        # pos class loss, we need to recompute the keep mask
        pred_logit = outputs["pred_pos_logits"].transpose(1,2)[tgt_idx] # re-compute the target order to matching order, redundant queries are ok
        # pred_logit 按照query target的顺序排列, 每个sample去了number spatial mask的数量, query target 跟ground truth顺序一致
        gt_logit = torch.zeros_like(pred_logit) # 按照target ground truth的顺序排列
    ...
    def loss_interleaves(self, outputs, targets, indices, num_masks, layer_id, extra):
        """Compute the losses related to the masks: the focal loss and the dice loss.
        targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
        """
        if layer_id >= self.top_x_layers['interleave']:
            return {"loss_interleave_itc_0": 0, "loss_interleave_ce_0": 0, "loss_interleave_dice_0": 0, "loss_interleave_bce_0": 0}

        losses = {}
        t_emb = outputs['pred_interleave_image'][:,-1]
        # t_emb = extra['interleave_classes'][:,-1]
        v_emb = outputs['pred_retrievals'][:,-1]

        loss_contrast = image_text_contrastive_loss_queue(v_emb, t_emb, extra['lang_encoder'], extra['training'])

        # return if there is no positive mask
        gt_masks = [x['interleave_masks'] for x in targets]
        _stack_gt_mask = torch.cat(gt_masks)
        if _stack_gt_mask.sum() == 0:
            return {"loss_interleave_itc_0": 0, "loss_interleave_ce_0": 0, "loss_interleave_dice_0": 0, "loss_interleave_bce_0": 0}
        ...
        outputs['pred_pos_logits'] = pred_logits # [bs, nq, 1]
        indices = self.matcher(outputs, targets, mode='interleave')
        src_idx = self._get_src_permutation_idx(indices)
        tgt_idx = self._get_tgt_permutation_idx(indices)
        ...
        gt_logit[_src_idx] = 1
        pred_logit = pred_logit[keep]
        loss_int_ce_pos = torch.sum(-gt_logit * F.log_softmax(pred_logit, dim=-1), dim=-1).mean()

        # TODO use valid to mask invalid areas due to padding in loss
        src_masks_pos = outputs["pred_imasks"][src_idx][keep]
        target_masks = target_masks.to(src_masks_pos)
        target_masks = target_masks[keep]
        ...
        num_masks = len(src_masks)
        losses.update({
            "loss_interleave_bce_0": sigmoid_ce_loss_jit(point_logits, point_labels, num_masks),
            "loss_interleave_dice_0": dice_loss_jit(point_logits, point_labels, num_masks),
        })
        losses.update({"loss_interleave_ce_0": loss_int_ce_pos, "loss_interleave_itc_0": loss_contrast})
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, num_masks, layer_id, extra):
        loss_map = {
            ...
            'retrievals_v2': self.loss_itc_v2,
            'interleaves': self.loss_interleaves,
